...
# сначала проверяем жёсткие несовместимости
# если позиция требует машину, а волонтёр её не имеет — исключаем
# если позиция требует ночные смены, а волонтёр их избегает — исключаем
# если позиция требует физическую работу, а волонтёр её не одобряет — исключаем
# если позиция требует тяжёлые подъёмы, а волонтёр их не одобряет — исключаем
# если позиция чувствительна к большим скоплениям людей, а волонтёр их избегает — исключаем
# если город не совпадает, а он указан — исключаем
# если район не совпадает, а он указан — исключаем
...
def build_features(position, volunteer) -> dict:
    ...
    if position.event.district_id and volunteer.district_id:
        ...
        location_match = 0.7
# Для локации используется не одна общая формула, а шкала:

# 1.0, если совпадает район;
# 0.8, если район не совпал, но совпадает город;
# 0.3, если город разный;
# 0.4, если проверяется только город и он не совпадает;
# 0.7, если данных о локации недостаточно.
